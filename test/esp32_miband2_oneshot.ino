...
bool connectToServer(BLEAddress pAddress) {
	...
	M5.Lcd.println("Connected to the device.");
	Serial.println("Connected to the device.");
	
	wait(1000);
	M5.Lcd.println("Getting services...");
	// ====================================================================
...
void setup() {
	...
	// M5.Lcd.println("Start after 5 seconds...");
	// delay(5000);
...
void loop() {
	if (flag_hrm) {
		pHRMControlCharacteristic->writeValue(HRM_ONESHOT_STOP, 3, true);
		pHRMControlCharacteristic->writeValue(HRM_ONESHOT_START, 3, true);
		Serial.println("# New one-shot command sent");
		flag_hrm = 0;
		wait(3000);
	} else {
		wait(.5);
